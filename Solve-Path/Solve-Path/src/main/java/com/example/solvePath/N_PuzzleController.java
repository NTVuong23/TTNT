...
public class N_PuzzleController implements Initializable, Runnable {
    ...
    private static final Logger logger = Logger.getLogger(N_PuzzleController.class.getName());
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        goal1Image.setImage(new Image(Objects.requireNonNull(DashboardApplication.class.getResourceAsStream("img/goal-1.png"))));
        goal2Image.setImage(new Image(Objects.requireNonNull(DashboardApplication.class.getResourceAsStream("img/goal-2.png"))));
        applyButtonAnimation(addImage);
        applyButtonAnimation(addNumber);
        applyButtonAnimation(compareBtn);
        applyButtonAnimation(jumbleBtn);
        applyButtonAnimation(playBtn);
        applyButtonAnimation(solveBtn);

    }
    private void applyButtonAnimation(Button button) {
        button.setOnMouseEntered(e -> createScaleTransition(button, 1.0, 1.0, 1.1, 1.1).play());
        button.setOnMouseExited(e -> createScaleTransition(button, 1.1, 1.1, 1.0, 1.0).play());
    }

    private ScaleTransition createScaleTransition(Button button, double fromX, double fromY, double toX, double toY) {
    ...
    public void run() {
        ...
        for (int i = 0; i <= totalStep; i++) {
            ...
            try {
                ...
                logger.log(Level.SEVERE, "Lỗi tràn bộ nhớ", e);
            }
    ...
    public void onChangeImageSize() {
        ...
        switch (selectedDiff.getId()) {
            case "medium" -> size = 4; // Kích thước 4x4
            case "hard" -> size = 5; // Kích thước 5x5
            default -> size = 3; // Kích thước 3x3
        }
    ...
    public void onSolveBtnClick() {
        ...
        if (!isSolve) {
            if(Objects.equals(algorithm, "BFS")) {
                BFS.stop = false;
            }else if (Objects.equals(algorithm, "DFS")) {
                DFS.stop = false;
            }else {
                AStar.stop = false;
            }
    ...
    // Giải quyết bài toán bằng thuật toán DFS
    public void solveDFS() {
        dFS = new DFS();
        dFS.startNode = new Node(state, 0);
        dFS.goalNode = new Node(goalState, 0);
        int maxDepth;
        if (size == 3) {
            maxDepth = 25;
        } else if (size == 4) {
        ...
        dFS.solve(maxDepth);
        result = dFS.RESULT;
        approvedNodes = dFS.approvedNodes;
        totalNodes = dFS.totalNodes;
        solveTime = dFS.time;
        error = dFS.error;
    }
    ...
    public Thread solveThread() {
        return new Thread(() -> {
            try {
                ...
                logger.log(Level.SEVERE, "Lỗi tràn bô nhớ", e);
    ...
    public void showWarning() {
        ...
        stage.getIcons().add(new Image(Objects.requireNonNull(DashboardApplication.class.getResourceAsStream("img/logo.png"))));
        DialogPane dialogPane = alert.getDialogPane();
        dialogPane.getStylesheets().add(Objects.requireNonNull(getClass().getResource("stylegame.css")).toExternalForm());
    ...
    public void showAlert() {
        ...
        if (Objects.equals(algorithm, "BFS")) {
            ...
            algorithmUsed = "A* với Heuristic " + State.heuristic;
        }

        alert.setContentText("Thuật toán sử dụng: " + algorithmUsed + "\n" +
    ...
    public void showCompare() {
        ...
        for (int i = 0; i < compareResults.size(); i++) {
            ...
            rsLabel.setStyle("-fx-border-color: black; -fx-border-width: 1px; -fx-padding: 5px;");
